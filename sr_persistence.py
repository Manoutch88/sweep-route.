...
class ContactManager:
    ...
    def build_index():
        ...
        name_idx, addr_idx, composite_idx, full_list = {}, {}, {}, []
    ...
    @staticmethod
    def update_contact(index, **kwargs):
        book = st.session_state.get("address_book", [])
        if 0 <= index < len(book):
            contact = book[index]
    ...
    @staticmethod
    def find_duplicate(name, address, exclude_index=-1, exact_match=False):
        book = st.session_state.get("address_book", [])
        if exact_match:
    ...
    @staticmethod
    def merge_contacts(t_idx, s_idx):
        book = st.session_state.get("address_book", [])
        if 0 <= t_idx < len(book) and 0 <= s_idx < len(book):
            t, s = book[t_idx], book[s_idx]
    ...
    @staticmethod
    def delete_contact_by_key(addr_key, name_key):
